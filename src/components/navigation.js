    ...
    render(){
        let menuContent = this.props.menuItems.filter((item)=>item.navItem).map((item,idx)=>{
            return <Nav.Item key={idx}>
                <Link to={item.path} className="nav-link fw-bold text-danger text-shadow">
                    {item.name}
                </Link>
            </Nav.Item>;
        });
        return(
            <Navbar bg="transparent" variant="light" expand="md" className="position-absolute px-2 w-100 z-index-100" collapseOnSelect>
                ...
                <Navbar.Collapse>
                    <Nav className="ms-auto text-end px-3 bg-md-white border-md-light-brw text-shadow opacity-9">
                        {menuContent}
                    </Nav>
                </Navbar.Collapse>
            </Navbar>
